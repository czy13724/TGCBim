...
```bash
wrangler d1 execute tgcbim --file=schema.sql --remote
```
...
2. 点击 **Create database**，命名为 `tgcbim`，点击 **Create**
...
1. 前往 [CF Dashboard](https://dash.cloudflare.com) → **Workers & Pages → tgcbim**（如没有，先部署一次）
...
**Q: 如何重置数据库？**  
A: 重新执行 `wrangler d1 execute tgcbim --file=schema.sql --remote`（⚠️ 会清空所有数据！）
